...
function FPSController({ modelRef, mobileInput, onLookAtChange, groundPlaneMesh, isMobile = false, boundaryLimits, initialPosition, initialYaw = 0, eyeHeight = 1.6, modelScale = 1 }) {
  ...
  useEffect(() => {
    ...
    modelRef.current.traverse((child) => {
      if (child.isMesh) {
        ...
        // Interactive objects
        if (name.startsWith('test') || name.includes('soggiorno') || name.includes('divano') || name.includes('tv')) {
  ...
  useFrame((_, delta) => {
    ...
    if (logTimerRef.current >= LOG_INTERVAL) {
      ...
      console.log(`[LivingRoomScene] 📷 Camera Y: ${cameraY.toFixed(2)} | Distance from ground: ${minGroundDistance.toFixed(2)} | Position: (${camera.position.x.toFixed(2)}, ${camera.position.y.toFixed(2)}, ${camera.position.z.toFixed(2)})`)
    }
  })
...
export default function LivingRoomScene({ onObjectClick, onLookAtChange, mobileInput, isMobile = false }) {
  ...
  useEffect(() => {
    ...
    console.log('[LivingRoom] Model spawnPoint:', modelRef.spawnPoint)
  ...
  const safeSpawnPosition = useMemo(() => {
    if (modelRef.spawnPoint) {
      ...
      console.log('[LivingRoom] Using GLB spawn point (world position):', spawnFromModel)
    ...
    if (boundaryLimits) {
      ...
      console.log('[LivingRoom] Spawning at room center (no GLB marker):', safeSpawn)
      return safeSpawn
    }
    
    const FALLBACK_SPAWN = { x: 0, y: 0, z: 0 }
    console.log('[LivingRoom] Using fallback spawn position:', FALLBACK_SPAWN)
    return FALLBACK_SPAWN
  }, [modelRef.spawnPoint, boundaryLimits])
  
  const initialYaw = useMemo(() => {
    const yaw = 0
    console.log('[LivingRoom] Initial yaw:', yaw, 'radians')
    return yaw
  }, [])
    ...
    <div style={{ width: '100%', height: '100%' }}>
      ...
      >
        ...
        <Suspense fallback={<LoadingIndicator />}>
          <CasaModel 
            sceneType="soggiorno"
            spawnNodeName="INIZIO_SOGGIORNO"
