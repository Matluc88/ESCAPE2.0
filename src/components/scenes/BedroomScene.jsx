  ...
  useEffect(() => {
    ...
    modelRef.current.traverse((child) => {
      if (child.isMesh) {
        ...
        if (name.startsWith('test') || name.includes('letto') || name.includes('armadio')) {
...
export default function BedroomScene({ onObjectClick, onLookAtChange, mobileInput, isMobile = false }) {
  ...
  useEffect(() => {
    ...
    console.log('[Bedroom] Model spawnPoint:', modelRef.spawnPoint)
  ...
  const safeSpawnPosition = useMemo(() => {
    console.log('[Bedroom] safeSpawnPosition RECALCULATED - modelRef:', {
      hasSpawnPoint: !!modelRef.spawnPoint,
      spawnPoint: modelRef.spawnPoint,
      hasBoundaryLimits: !!boundaryLimits
    });
    ...
    if (boundaryLimits) {
      ...
      console.log('[Bedroom] ⚠️ Spawning at room center (no GLB marker):', safeSpawn)
      return safeSpawn
    }
  ...
  const initialYaw = useMemo(() => {
    const yaw = 0
    console.log('[Bedroom] Initial yaw:', yaw, 'radians')
    return yaw
  }, [])
    ...
    <div style={{ width: '100%', height: '100%' }}>
      ...
      >
        ...
        <Suspense fallback={<LoadingIndicator />}>
          <CasaModel 
            sceneType="camera"
            spawnNodeName="INIZIO_CAMERA_DA_LETTO"
